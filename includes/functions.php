 ...
 */ 
function getCalender($year = '2020', $month = '09'){ 
...
?> 
    ...
    $result = mysqli_query($con, "SELECT * FROM tb_id_medical_info WHERE mi_init_date='$date'"); 
     if(mysqli_num_rows($result) > 0){ 
         ...
         while($row = mysqli_fetch_assoc($result)){ 
             $i++; 
             $eventListHTML .= '<li class="sidebar__list-item"><span class="list-item__time">'.$i.'.</span>'.$row['miid'].'</li>'; 
         } 
         $eventListHTML .= '</ul>'; 
     } 
     echo $eventListHTML;
